...
Q[4] <- sum.finite(log(pi1)+log(dnorm(x, mu1, sigma1))) + sum.finite(log(pi2)+log(dnorm(x, mu2, sigma2))) + sum.finite(log(pi3)+log(dnorm(x, mu3, sigma3))) + sum.finite(log(pi4)+log(dnorm(x, mu4, sigma4)))
Q[4]
k <- 4
...
hist(x, prob=T, breaks=32, xlim=c(range(x)[1], range(x)[2]), main='4-Component Gaussian Mixture')
